...
* Also taken more or less as-is from Wojciech Mula's respository
****************/
 ...
 *   at least generic_base64_encode_len(len) bytes (see below)
 ...
 * char* dest = (char*) malloc(generic_base64_decode_len(srclen));
 * int len = generic_base64_encode(dest, src, sourcelen);
 ...
 *
 */
size_t generic_base64_encode(char* dest, const char* str, size_t len);
 ...
 * char* dest = (char*) malloc(generic_base64_encode_len(srclen));
 * int len = generic_base64_decode(dest, src, sourcelen);
 * if (len == MODP_B64_ERROR) { error }
 * \endcode
 */
size_t generic_base64_decode(char* dest, const char* src, size_t len);
 ...
 */
#define generic_base64_encode_len(A) ((A+2)/3 * 4 + 1)
 ...
 * floor[len * 3/4] + 2
 */
#define generic_base64_decode_len(A) (A / 4 * 3 + 2)
 ...
 * if (generic_base64_encode_strlen(sizeof(struct datastuff)) != len) {
 *    // wrong size
 *    return false;
 * } else {
 *    // safe to do;
 *    if (generic_base64_encode((char*) &foo, b64encoded, len) == MODP_B64_ERROR) {
 *      // bad characters
 *      return false;
 *    }
 * }
 ...
 * \endcode
 */
#define generic_base64_encode_strlen(A) ((A + 2)/ 3 * 4)
 ...
 */
inline std::string& generic_base64_encode(std::string& s)
{
    std::string x(generic_base64_encode_len(s.size()), '\0');
    size_t d = generic_base64_encode(const_cast<char*>(x.data()), s.data(), (int)s.size());
    if (d == MODP_B64_ERROR) {
        x.clear();
    } else {
